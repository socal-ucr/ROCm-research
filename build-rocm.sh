...
if [ ! -d "${INSTALL_DIR}/rocm/.info" ]; then
    ...
    echo 5.2.0-65 > ${INSTALL_DIR}/rocm/.info/version
    echo 5.2.0-65 > ${INSTALL_DIR}/rocm/.info/version-dev
    echo 5.2.0-65 > ${INSTALL_DIR}/rocm/.info/version-utils
fi
...
if [ ! -f "${INSTALL_DIR}/rocm/lib/libamd_comgr.so.2.4" ]; then
    echo "LINKING hip/lib/libamd_comgr.so"
    ln -s ${INSTALL_DIR}/rocm/comgr/lib/libamd_comgr.so.2.4 ${INSTALL_DIR}/rocm/lib/.
