...
rm -rf build
mkdir build && cd build
CC=${CC_DIR} CXX=${CXX_DIR} cmake -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR}/rocm -DCMAKE_PREFIX_PATH="${INSTALL_DIR}/rocm/include;${INSTALL_DIR}/rocm/lib;${INSTALL_DIR}/rocm/rocdl" DCMAKE_C_FLAGS=-DNDEBUG -DCMAKE_CXX_FLAGS=-DNDEBUG ..
make -j
make install
cd ${cwd}
...
#echo 4.1.0 > ${INSTALL_DIR}/rocm/.info/version
#echo 4.1.0 > ${INSTALL_DIR}/rocm/.info/version-dev
#echo 4.1.0 > ${INSTALL_DIR}/rocm/.info/version-utils
